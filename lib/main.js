...
var rgumbel = require( '@stdlib/random-base-gumbel' ).factory;
...
function iterator( mu, beta, options ) {
	...
	if ( !isNumber( mu ) || isnan( mu ) ) {
		throw new TypeError( format( '0pn6v,NI', mu ) );
	}
	if ( !isPositive( beta ) ) {
		throw new TypeError( format( '0pn72,Go', beta ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pn2V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pn2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
